...
`ASTROLABE_API_KEY` is your own random shared secret (not your OpenRouter key). Use this same value in client auth requests.
...
For the full routing tree and threshold logic, see [Routing Reference](/routing-reference).
For model costs and benchmark confidence labels, see [Model Roster Reference](/model-roster).
</Note>
