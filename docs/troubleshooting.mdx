...
1. Restarted Astrolabe after editing env vars
2. Edited the correct runtime environment (local `.env` vs deployment env)
3. `GET /health` reflects expected normalized values for `routing_profile` and `cost_efficiency_mode`
4. Variable names and values match `/configuration` exactly
